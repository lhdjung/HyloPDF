...
1. **Move `dioxus-reader` to a git dependency on `c6dec888`.** An afternoon.
   QRnew's `Cargo.toml` is the template. Nothing else in this list is possible
   until a fresh checkout builds.
2. **Add the three-platform CI job on `cargo test`.** No GPU, no screen, and it
   is the only thing that will say whether Stylo, Parley and fontique behave on
   engines this was not developed on. Highest information per hour in the whole
   experiment.
3. **Write the composed-input test and retire the IME decision.** An hour, and
   it removes the item both documents call blocking.
4. **File the focus fault upstream** with the `is_focussable()` pointer from
   ...
   call in a file that already has the predicate.
