...
fn themes_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../src-tauri/themes")
}
...
fn write_built_in_table() {
    let dir = themes_dir();
    println!("cargo:rerun-if-changed=../../src-tauri/themes");
    ...
    for (_, id) in &themes {
        let _ = writeln!(
            out,
            "    ({id:?}, include_str!(concat!(env!(\"CARGO_MANIFEST_DIR\"), \"/../../src-tauri/themes/{id}.toml\"))),"
        );
    }
    out.push_str("];\n");
