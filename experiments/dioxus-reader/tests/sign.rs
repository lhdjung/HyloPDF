...
/// their document.
///
/// The fixture is the app's own `tests/fixtures/signed.pdf`, which is the only
/// signed document in this repository. If it is not there — it is generated by
/// the app's test run rather than committed — the test says so and stops
/// rather than passing on nothing.
#[test]
fn a_signed_document_says_what_signing_it_costs() {
    let theirs = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/fixtures/signed.pdf");
    if !theirs.exists() {
        eprintln!("no signed fixture at {} — run the app's `npm test` once", theirs.display());
        return;
    }
    let dir = std::env::temp_dir().join(format!("hylopdf-signed-{}", std::process::id()));
    std::fs::create_dir_all(&dir).expect("a directory");
    let path = dir.join("carries-a-signature.pdf");
    std::fs::copy(&theirs, &path).expect("a copy to write to");
