...
This is the largest item on the list by cost, because the app's state is one
`App` object and one window. Two viable shapes: a second Tauri window sharing
...
| 2.11 | **Remappable keys** | Only j/k are offered to the Vim-shaped reader — no h/l, no gg/G, no ctrl-d/u, and nothing is remappable. `AGENTS.md` already names a keybindings file as future work; the audience that cares about this is exactly the audience that would otherwise use Zathura or Sioyek | Medium |
...
**Very high zoom goes soft.** `MAX_CANVAS_PIXELS` is 12 million
