...
test("the ink on a selection is the inverse of the area under it", () => {
  const ink = selectionInk(theme({ selection_area: "#802020" }));
...
test("an inverse too close to its own ground gives way to black or white", () => {
  ...
  const ink = toHex(selectionInk(theme({ selection_area: "#808080" })));
  assert.equal(ink, "#000000");
  assert.equal(toHex(selectionInk(theme({ selection_area: "#6b6b6b" }))), "#ffffff");
});

test("every derived pair is legible on itself", () => {
  for (const selection_area of [null, "#4a2f6b", "#44475a", "#504945", "#c0d1d4"]) {
    for (const background of ["#24272f", "#f2f1ed", "#010105"]) {
      const it = theme({ selection_area, background });
      const ratio = contrastRatio(selectionInk(it), selectionArea(it));
      assert.ok(ratio >= 3, `${selection_area} on ${background} came out at ${ratio.toFixed(2)}:1`);
    }
  }
});

test("a theme that names both is taken at its word", () => {
  const it = theme({ selection_area: "#123456", selection_text: "#fedcba" });
...
test("a theme says which of its colours could not be read", () => {
  ...
  assert.deepEqual(unreadableColors(theme({ accent: null, selection_area: null })), []);
});
