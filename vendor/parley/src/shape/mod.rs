    ...
    while let Some(font) = current_font.take() {
        ...
        let glyph_buffer = harf_shaper.shape(
            ...
            ShapeOptions::new()
                ...
                // words in WebKit. Divide the scale back out: tracking is a
                // property of the size the reader sees, not of the pixels.
                .point_size(Some(item.size / layout.data.scale)),
        );
